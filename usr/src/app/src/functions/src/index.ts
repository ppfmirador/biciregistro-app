...
// enforceAppCheck: false is CRITICAL for local development to bypass App Check errors
// that manifest as CORS issues. This should be set to 'true' for production.
setGlobalOptions({ region: "us-central1", enforceAppCheck: false });
...
const callOptions = {
  // CORS is now handled by the global options unless specific overrides are needed
  // which they are not in this case. The 'cors' property can be removed.
};
