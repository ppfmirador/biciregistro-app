...
import { ArrowLeft, CalendarDays, Bike as BikeIcon, Tag, Palette, Info, ShieldAlert, History, Image as ImageIcon, UploadCloud, Edit, Trash2, Paperclip, Download, FileText, MapPin, CheckCircle, RefreshCw, Loader2, Cog, UserCircle, QrCode } from 'lucide-react';
...
function BikeDetailsPageContent() {
  ...
  const [isRecoverConfirmationOpen, setIsRecoverConfirmationOpen] = useState(false);
  ...
  const serialNumberParam = React.useMemo(() => {
    if (typeof serialNumberFromParams === 'string' && serialNumberFromParams.trim() !== '') {
        return serialNumberFromParams.trim();
    }
    if (Array.isArray(serialNumberFromParams) && serialNumberFromParams.length > 0 && typeof serialNumberFromParams[0] === 'string' && serialNumberFromParams[0].trim() !== '') {
  ...
  const fetchBike = React.useCallback(async () => {
    if (serialNumberParam) {
      ...
      try {
        ...
        setBike(bikeData);
        if (bikeData && user) {
          setIsOwner(bikeData.ownerId === user.uid);
        } else {
          setIsOwner(false);
        }
        if (!bikeData) {
      ...
      // This case should be caught by the early return if serialNumberParam is null
  ...
  useEffect(() => {
    if (serialNumberParam && (!authLoading || user)) { // Check authLoading or if user is available
       fetchBike();
    } else if (!serialNumberParam && !authLoading) {
      // If serialNumberParam is definitively null (not just initial undefined from router)
      // and auth is not loading, set an error.
      setPageError("Número de serie no válido en la URL.");
      setLoading(false);
    }
  }, [serialNumberParam, authLoading, user, fetchBike]);
  
  useEffect(() => {
    const fetchOwnerContactInfo = async () => {
      // Fetch contact info only if the user is a bikeshop and we have a bike with an owner
      if (user?.role === 'bikeshop' && bike?.ownerId) {
        try {
          const ownerDoc = await getUserDoc(bike.ownerId);
          if (ownerDoc) {
            setBike(prevBike => {
              if (!prevBike) return null;
              return {
                ...prevBike,
                ownerEmail: ownerDoc.email || '',
                ownerWhatsappPhone: ownerDoc.whatsappPhone || ''
              };
            });
          }
        } catch (error) {
          console.warn(`Could not fetch owner contact info for owner ${bike.ownerId}`, error);
        }
      }
    };

    if (bike) { // Only run if bike data is available
        fetchOwnerContactInfo();
    }
  }, [bike, user]); // Re-run if the bike data or the user changes
      ...
      return (
        <div className="text-center py-10">
          <Alert variant="destructive" className="max-w-md mx-auto">
            ...
            <AlertDescription>El número de serie proporcionado en la URL no es válido o está ausente.</AlertDescription>
          </Alert>
  ...
  const handleNextImage = () => {
    if (bike && bike.photoUrls && bike.photoUrls.length > 1) {
      setCurrentImageIndex((prevIndex) => (prevIndex + 1) % bike.photoUrls!.length);
    }
  };

  const handlePrevImage = () => {
    if (bike && bike.photoUrls && bike.photoUrls.length > 1) {
      setCurrentImageIndex((prevIndex) => (prevIndex - 1 + bike.photoUrls!.length) % bike.photoUrls!.length);
    }
  };

  const handlePhotoFileChange = async (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
    if (!user) {
        toast({ title: 'No Autenticado', description: 'Debes iniciar sesión para subir una foto.', variant: 'destructive' });
        return;
    }
    if (!event.target.files || event.target.files.length === 0 || !bike || bike.ownerId !== user.uid) {
      return;
    }
    ...
    try {
      ...
      const finalPhotoUrls = newPhotoUrls.filter(url => url && url.trim() !== '');
      ...
      console.error(`Error uploading photo ${index + 1}:`, uploadError);
      toast({ title: 'Error al Subir Foto', description: uploadError.message || `No se pudo subir la foto ${index + 1}.`, variant: 'destructive' });
    } finally {
      setUploadingPhoto(prev => {
        const newUploading = [...prev];
  ...
  const handleRemovePhoto = async (index: number) => {
    if (!user) {
        toast({ title: 'No Autenticado', description: 'Debes iniciar sesión para eliminar una foto.', variant: 'destructive' });
        return;
    }
    if (!bike || bike.ownerId !== user.uid || !bike.photoUrls || !bike.photoUrls[index]) {
      return;
    }
     setUploadingPhoto(prev => {
      const newUploading = [...prev];
    ...
    try {
      ...
      toast({ title: 'Error al Eliminar Foto', description: error.message || `No se pudo eliminar la foto ${index + 1}.`, variant: 'destructive' });
    } finally {
       setUploadingPhoto(prev => {
        const newUploading = [...prev];
        newUploading[index] = false;
  ...
  const handleDocumentFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!user) {
        toast({ title: 'No Autenticado', description: 'Debes iniciar sesión para subir un documento.', variant: 'destructive' });
        return;
    }
    if (!event.target.files || event.target.files.length === 0 || !bike || bike.ownerId !== user.uid) {
      return;
    ...
    try {
      ...
      toast({ title: 'Error al Subir Documento', description: uploadError.message || 'No se pudo subir el documento.', variant: 'destructive' });
    } finally {
  ...
  const handleRemoveDocument = async () => {
    if (!user) {
        toast({ title: 'No Autenticado', description: 'Debes iniciar sesión para eliminar un documento.', variant: 'destructive' });
        return;
    }
    if (!bike || bike.ownerId !== user.uid || !bike.ownershipDocumentUrl) {
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Error al Eliminar Documento', description: error.message || 'No se pudo eliminar el documento.', variant: 'destructive' });
    } finally {
  ...
  if (pageError && !loading) { // Only show pageError if not also loading
    return (
      <div className="text-center py-10">
         <Alert variant="destructive" className="max-w-md mx-auto">
          <ShieldAlert className="h-4 w-4" />
  ...
  if (!bike && !loading && !pageError) { // Bike not found, but no specific error and not loading
     return (
      <div className="text-center py-10">
        <p className="text-xl text-muted-foreground">Bicicleta no encontrada.</p>
  ...
  const displayPhotoUrl = bike.photoUrls && bike.photoUrls.length > 0 && bike.photoUrls[currentImageIndex]
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-2 mb-6">
        <Button variant="outline" onClick={backButtonAction} className="w-full sm:w-auto">
          <ArrowLeft className="mr-2 h-4 w-4" /> {backButtonText}
        </Button>
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          {isOwner && (
            <Link href={`/bike/${bike.serialNumber}/qr`} passHref className="w-full sm:w-auto">
              <Button variant="outline" className="w-full">
                <QrCode className="mr-2 h-4 w-4" /> Gestionar QR
              </Button>
            </Link>
          )}
          {isOwner && (bike.status === BIKE_STATUSES[0] || bike.status === BIKE_STATUSES[1]) && (
            <Link href={`/bike/${bike.serialNumber}/edit`} passHref className="w-full sm:w-auto">
      ...
      <Card className="shadow-xl overflow-hidden">
        <CardHeader className="bg-muted/50 p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4">
            <div>
              <CardTitle className="text-2xl sm:text-3xl font-headline text-primary">{bike.brand} {bike.model}</CardTitle>
              <CardDescription className="text-base sm:text-lg">Número de Serie: {bike.serialNumber}</CardDescription>
            </div>
            <div className="flex flex-col items-start sm:items-end gap-2">
              <BikeStatusBadge status={bike.status} className="px-3 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold" />
              {isOwner && bike.status === BIKE_STATUSES[1] && (
                ...
                >
                  {markingRecovered ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle className="mr-2 h-4 w-4" />}
          ...
          <div className="md:col-span-1 relative">
            ...
            {(!bike.photoUrls || numPhotos === 0) && !loading && (
                 <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/80 rounded-lg">
                    ...
                    <p className="mt-2 text-xs sm:text-sm text-muted-foreground">Sin imagen disponible</p>
                 </div>
            )}
          </div>
          <div className="md:col-span-2 space-y-3 sm:space-y-4">
            <h3 className="text-lg sm:text-xl font-semibold text-foreground mb-2 border-b pb-2">Detalles de la Bicicleta</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <DetailItem icon={<BikeIcon className="h-5 w-5 text-primary" />} label="Marca" value={bike.brand} />
              <DetailItem icon={<Tag className="h-5 w-5 text-primary" />} label="Modelo" value={bike.model} />
              <DetailItem icon={<Cog className="h-5 w-5 text-primary" />} label="Tipo" value={bike.bikeType || 'N/A'} />
              <DetailItem icon={<Palette className="h-5 w-5 text-primary" />} label="Color" value={bike.color || 'N/A'} />
              <DetailItem icon={<MapPin className="h-5 w-5 text-primary" />} label="Estado Principal (México)" value={bike.state || 'N/A'} />
              <DetailItem icon={<CalendarDays className="h-5 w-5 text-primary" />} label="Registrada el" value={format(new Date(bike.registrationDate), 'PPP', { locale: es })} />
              <DetailItem
            ...
            {bike.description && <DetailItem icon={<Info className="h-5 w-5 text-primary" />} label="Descripción" value={bike.description} fullWidth />}
            ...
            {bike.status === BIKE_STATUSES[1] && (
              <Alert variant="destructive" className="mt-4">
                ...
                <AlertDescription className="space-y-1 text-xs sm:text-sm">
                  <p>Último cambio de estado: {format(new Date(bike.statusHistory[bike.statusHistory.length -1].timestamp), 'PPP p', { locale: es })}</p>
                  {bike.statusHistory[bike.statusHistory.length-1].notes && <p className="text-xs">Notas del reporte: {bike.statusHistory[bike.statusHistory.length-1].notes}</p>}

                  {bike.theftDetails && (
                    <div className="mt-2 pt-2 border-t border-destructive/30 text-xs space-y-0.5">
                      <p><strong>Lugar del robo (Estado):</strong> {bike.theftDetails.theftLocationState}</p>
                      <p><strong>Detalles del incidente:</strong> {bike.theftDetails.theftIncidentDetails}</p>
                      {bike.theftDetails.theftPerpetratorDetails && <p><strong>Detalles del perpetrador:</strong> {bike.theftDetails.theftPerpetratorDetails}</p>}
                      <p><strong>Fecha del reporte:</strong> {format(new Date(bike.theftDetails.reportedAt), 'PPP p', { locale: es })}</p>
                    </div>
                  )}
                   <p className="mt-2 font-semibold">Si tienes información, contacta a las autoridades.</p>
                </AlertDescription>
              </Alert>
            )}
          </div>
        ...
        {user && user.role === 'bikeshop' && bike.ownerId && (bike.ownerFirstName || bike.ownerEmail || bike.ownerWhatsappPhone) && (
          <>
            <Separator />
            <CardContent className="p-4 sm:p-6">
              ...
              <div className="space-y-2 text-sm">
                {(bike.ownerFirstName || bike.ownerLastName) && (
                  <p><strong>Nombre:</strong> {bike.ownerFirstName || ''} {bike.ownerLastName || ''}</p>
                )}
                {bike.ownerEmail && (
                  <p><strong>Correo Electrónico:</strong> <a href={`mailto:${bike.ownerEmail}`} className="text-primary hover:underline">{bike.ownerEmail}</a></p>
                )}
                {bike.ownerWhatsappPhone && (
                  <p><strong>Teléfono WhatsApp:</strong> <a href={`https://wa.me/${bike.ownerWhatsappPhone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{bike.ownerWhatsappPhone}</a></p>
                )}
                {!bike.ownerFirstName && !bike.ownerLastName && !bike.ownerEmail && !bike.ownerWhatsappPhone && (
                    <p className="text-muted-foreground">No hay información de contacto adicional disponible para el propietario.</p>
                )}
              </div>
            </CardContent>
          </>
            ...
            <CardContent className="p-4 sm:p-6 space-y-6">
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {[0, 1, 2].map((index) => (
                    <Card key={`photo-manage-${index}`} className="p-3 sm:p-4 space-y-2">
                      <p className="text-xs sm:text-sm font-medium text-center">Foto {index + 1}</p>
                          ...
                          <Image
                            ...
                            onError={(e) => {
                                ...
                                if(parent){
                                  ...
                                  placeholderText.className = 'text-xs text-destructive-foreground p-2';
                      ...
                      >
                        {uploadingPhoto[index] ? 'Subiendo...' : (bike.photoUrls && bike.photoUrls[index] ? 'Cambiar' : 'Agregar')}
                        {!uploadingPhoto[index] && <UploadCloud className="ml-2 h-4 w-4" />}
                      </Button>
                      {bike.photoUrls && bike.photoUrls[index] && (
                        <Button
                          variant="ghost"
              ...
              <div>
                ...
                {bike.ownershipDocumentUrl && bike.ownershipDocumentName ? (
                  <Card className="p-3 sm:p-4 space-y-3 bg-muted/50">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 mt-2">
                      ...
                      >
                        ...
                        {!uploadingDocument && <UploadCloud className="ml-2 h-4 w-4" />}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                  ...
                  <Card className="p-4 text-center border-dashed">
                    <p className="text-xs sm:text-sm text-muted-foreground mb-3">No se ha subido ningún documento de propiedad.</p>
                    <Button
                        variant="default"
                        onClick={() => documentInputRef.current?.click()}
          ...
          <>
            ...
            <CardContent className="p-4 sm:p-6">
              ...
              <ul className="space-y-3">
                {bike.statusHistory.slice().reverse().map((entry: StatusEntry, histIndex: number) => (
                  <li key={histIndex} className="p-3 bg-muted/30 rounded-md border border-border/50 text-xs sm:text-sm">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-1 gap-1 sm:gap-0">
                      <BikeStatusBadge status={entry.status} className="self-start"/>
                      <span className="text-xs text-muted-foreground">{format(new Date(entry.timestamp), 'PPP p', { locale: es })}</span>
                    </div>
                    {entry.notes && <p className="text-muted-foreground text-xs mt-1 italic break-words">Notas: {entry.notes}</p>}
                    {entry.status === BIKE_STATUSES[2] && entry.transferDocumentUrl && entry.transferDocumentName && (
                      <div className="mt-2 pt-2 border-t border-muted-foreground/20">
                        ...
                        >
                          ...
                          Documento de Transferencia: {entry.transferDocumentName}
                        </a>
                      </div>
                    )}
                  </li>
      ...
      <AlertDialog open={isRecoverConfirmationOpen} onOpenChange={setIsRecoverConfirmationOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={markingRecovered}>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleMarkAsRecovered} disabled={markingRecovered} className="bg-green-600 hover:bg-green-700">
              {markingRecovered && <RefreshCw className="mr-2 h-4 w-4 animate-spin" />}
...
const DetailItem: React.FC<DetailItemProps> = ({ icon, label, value, fullWidth }) => (
  <div className={`flex items-start space-x-2 sm:space-x-3 ${fullWidth ? 'col-span-1 sm:col-span-2' : ''}`}>
    <span className="mt-1 flex-shrink-0">{icon}</span>
    <div>
      <p className="text-xs sm:text-sm font-medium text-muted-foreground">{label}</p>
      <p className="text-sm sm:text-base text-foreground font-semibold break-words">{value}</p>
    </div>
  </div>
);
