...
import { Loader2, ArrowLeft, Bike as BikeIcon, PlusCircle, Search as SearchIcon, Eye } from 'lucide-react';
  ...
  return (
    <Card className="flex flex-col sm:flex-row sm:items-center p-3 sm:p-4 gap-3 sm:gap-4 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex-grow space-y-1">
        <h3 className="font-semibold text-base sm:text-lg">{bike.brand} {bike.model}</h3>
        <p className="text-xs sm:text-sm text-muted-foreground">N/S: {bike.serialNumber}</p>
        <p className="text-xs sm:text-sm">
          Propietario: {bike.ownerFirstName || ''} {bike.ownerLastName || ''}
          {bike.ownerEmail && <span className="text-muted-foreground text-xs"> ({bike.ownerEmail})</span>}
        </p>
      </div>
...
export default function BikeShopInventoryPage() {
  ...
  const fetchShopBikes = useCallback(async (currentSearchTerm: string) => {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user || user.role !== 'bikeshop') {
        toast({ title: "Acceso Denegado", description: "Debes ser una tienda para ver esta página.", variant: "destructive" });
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <Card className="shadow-md">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Buscar Bicicletas</CardTitle>
          <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row items-start sm:items-center gap-2 pt-2">
            <Input
              type="text"
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <Button type="submit" variant="outline" disabled={isLoading} className="w-1/2 sm:w-auto">
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <SearchIcon className="mr-2 h-4 w-4" />}
                Buscar
              </Button>
              {searchTerm && (
                <Button variant="ghost" onClick={handleClearSearch} disabled={isLoading} className="w-1/2 sm:w-auto">
                  Limpiar
                </Button>
              )}
            </div>
        ...
        <CardContent>
          ...
          {!isLoading && bikes.length === 0 && searchTerm ? (
            ...
            <div className="text-center py-10">
              ...
              <CardTitle className="text-xl sm:text-2xl">Aún No Has Registrado Bicis</CardTitle>
              <CardDescription className="mb-6 text-sm sm:text-base">
                Cuando registres bicicletas vendidas a tus clientes, aparecerán aquí.
              </CardDescription>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              {!searchTerm && bikes.length === 10 && (
                <p className="text-center text-sm text-muted-foreground pt-4">
                  Mostrando las últimas 10 bicicletas. Usa la búsqueda para encontrar más.
                </p>
              )}
            </div>
          )}
