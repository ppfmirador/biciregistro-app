...
import {
  bikeShopRegisterSchema,
  type BikeShopRegisterFormValues,
} from "@/lib/schemas";
...
export default function RegisterSoldBikePage() {
  ...
  const [verifiedCustomer, setVerifiedCustomer] = useState<UserProfile | null>(
    null,
  );
  const [customerNotFound, setCustomerNotFound] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [enteredCustomerEmail, setEnteredCustomerEmail] = useState("");
  const [showPostRegistrationMessage, setShowPostRegistrationMessage] =
    useState(false);
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<BikeShopRegisterFormValues>({
    ...
    if (!authLoading) {
      if (!user || user.role !== "bikeshop") {
        toast({
          ...
          description: "Debes ser una tienda para acceder.",
          variant: "destructive",
        });
  ...
  const handleVerifyCustomerEmail = async () => {
    if (!watchedCustomerEmail) {
      setError("customerEmail", {
        type: "manual",
        message: "Ingresa un correo para verificar.",
      });
      return;
    }
    ...
    try {
      ...
      if (foundUser) {
        ...
        toast({
          ...
          description: `Cliente ${foundUser.email} verificado.`,
        });
      } else {
        ...
        toast({
          ...
          description:
            "Este cliente no tiene una cuenta. Ingresa sus datos manualmente para crear una pre-cuenta.",
          variant: "default",
        });
      }
      ...
      toast({
        ...
        description: "No se pudo verificar el correo del cliente.",
        variant: "destructive",
      });
    } finally {
    ...
    try {
      if (verifiedCustomer) {
        ...
        if (!data.customerFirstName || !data.customerLastName) {
          setError("customerFirstName", {
            type: "manual",
            message: "Nombre es obligatorio.",
          });
          setError("customerLastName", {
            type: "manual",
            message: "Apellido es obligatorio.",
          });
          toast({
            title: "Datos Faltantes",
            description: "Completa nombre y apellido del nuevo cliente.",
            variant: "destructive",
          });
        ...
        const newCustomerDetails = {
          ...
          gender:
            data.customerGender as BikeShopRegisterFormValues["customerGender"],
          registeredByShopId: user.uid,
        };
        const { uid } = await createAccountByAdmin(
          newCustomerDetails,
          "cyclist",
          user.uid,
        );
        ...
        toast({
          title: "Error",
          description: "Verifica o ingresa los datos del cliente.",
          variant: "destructive",
        });
        setIsSubmitting(false);
        return;
      ...
      if (!customerUidToAssign) {
        toast({
          title: "Error Crítico",
          description: "No se pudo determinar el UID del cliente.",
          variant: "destructive",
        });
        setIsSubmitting(false);
        return;
      ...
      const processUpload = async (
        ...
        if (fileList && fileList.length > 0) {
          ...
          if (type === "photo") {
            ...
            return {
              url: await uploadFileToStorage(file, filePath),
              name: file.name,
            };
          }
        }
      ...
      const photoUploadPromises = [data.photo1, data.photo2, data.photo3].map(
        (fileList, i) => {
          if (fileList && fileList.length > 0) {
      ...
      const ownershipDocResult = await processUpload(
        data.ownershipDocument,
        "document",
      );

      const finalBrand =
        data.brand === OTHER_BRAND_VALUE ? data.otherBrand || "" : data.brand;
      ...
      if (customerNotFound) {
        // If a new pre-account was created
      ...
      const errorMessage =
        err.message ||
        "No se pudo registrar la bicicleta. Revisa los datos e inténtalo de nuevo.";
      console.error("Error registering bike by shop:", _error);
      if (
        errorMessage.includes(
          "Ya existe una bicicleta registrada con el número de serie:",
        )
      ) {
        setError("serialNumber", { type: "manual", message: errorMessage });
        toast({
          title: "Error: Número de Serie Duplicado",
    ...
    <div className="space-y-6">
      ...
      <Card className="max-w-3xl mx-auto shadow-xl">
        <CardHeader>
          ...
          <CardDescription>
            Ingresa el correo del cliente para verificar si tiene cuenta. Luego,
            completa los datos de la bicicleta.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <section className="space-y-4 p-4 border rounded-lg bg-card">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    type="button"
                    onClick={handleVerifyCustomerEmail}
                    disabled={
                      isVerifyingCustomer ||
              ...
              {verifiedCustomer && !customerNotFound && (
                <Card className="p-3 bg-green-50 border-green-200">
                  ...
                  <ul className="text-xs text-green-600 list-disc list-inside pl-1 space-y-0.5">
                    ...
                    {verifiedCustomer.country && (
                      <li>
                        <span className="font-semibold">País:</span>{" "}
                        {verifiedCustomer.country}
                      </li>
                    )}
                    {verifiedCustomer.profileState && (
                      <li>
                        <span className="font-semibold">Estado:</span>{" "}
                        {verifiedCustomer.profileState}
                      </li>
                    )}
                    {verifiedCustomer.postalCode && (
                      <li>
                        <span className="font-semibold">C.P.:</span>{" "}
                        {verifiedCustomer.postalCode}
                      </li>
                    )}
                    {verifiedCustomer.whatsappPhone && (
                      <li>
                    ...
                    {verifiedCustomer.gender && (
                      <li>
                        ...
                        {GENDERS.find(
                          (g) => g.value === verifiedCustomer.gender,
                        )?.label || verifiedCustomer.gender}
                      </li>
                    )}
                  </ul>
                  <p className="text-xs text-green-700 mt-2">
                    Los datos del cliente no son editables aquí. La bicicleta se
                    asignará a este usuario.
                  </p>
                </Card>
              )}
              ...
              {customerNotFound && (
                <div className="space-y-4 pt-3 border-t">
                  <p className="text-sm font-medium text-foreground">
                    Ingresa los datos del nuevo cliente (se creará una
                    pre-cuenta):
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className={
                          errors.customerFirstName ? "border-destructive" : ""
                        }
                        disabled={isSubmitting}
                      />
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className={
                          errors.customerLastName ? "border-destructive" : ""
                        }
                        disabled={isSubmitting}
                      />
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Select
                            onValueChange={(value) =>
                              field.onChange(
                                value === PLACEHOLDER_NONE_VALUE ? "" : value,
                              )
                            }
                            value={field.value || ""}
                            disabled={isSubmitting}
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="customerProfileState">
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Select
                            ...
                            value={field.value || ""}
                            disabled={isSubmitting || !watchedCustomerCountry}
                          >
                            <SelectTrigger>
                              <SelectValue
                                placeholder={
                                  !watchedCustomerCountry
                            ...
                            <SelectContent>
                              ...
                              {LAT_AM_LOCATIONS.find(
                                (c) => c.country === watchedCustomerCountry,
                              )?.states.map((s) => (
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className={
                          errors.customerPostalCode ? "border-destructive" : ""
                        }
                        disabled={isSubmitting}
                      ...
                      {errors.customerPostalCode && (
                        <p className="text-xs text-destructive">
                          {errors.customerPostalCode.message}
                        </p>
                      )}
                    </div>
                    <div className="space-y-1">
                      <Label htmlFor="customerWhatsappPhone">
                        Teléfono WhatsApp (Opcional)
                      </Label>
                      <Input
                        id="customerWhatsappPhone"
                        {...register("customerWhatsappPhone")}
                        className={
                          errors.customerWhatsappPhone
                            ? "border-destructive"
                            : ""
                        }
                        disabled={isSubmitting}
                      ...
                      {errors.customerWhatsappPhone && (
                        <p className="text-xs text-destructive">
                          {errors.customerWhatsappPhone.message}
                        </p>
                      )}
                    </div>
                    <div className="space-y-1">
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Select
                            onValueChange={(value) =>
                              field.onChange(
                                value === PLACEHOLDER_NONE_VALUE
                                  ? ""
                                  : (value as BikeShopRegisterFormValues["customerGender"]),
                              )
                            }
                            value={field.value || ""}
                            disabled={isSubmitting}
            ...
            <section className="space-y-4 p-4 border rounded-lg bg-card">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <Label htmlFor="bikeState">
                    Estado (Ubicación Bici - Opcional)
                  </Label>
                  <Controller
                    name="bikeState"
                    control={control}
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  ...
                  placeholder="Detalles como componentes, accesorios, etc."
                  disabled={isSubmitting}
                />
              </div>

              <h4 className="text-md font-medium pt-3 border-t">
              ...
              {[1, 2, 3].map((i) => (
                <div className="space-y-1" key={`photo-upload-${i}`}>
                  ...
                  <Input
                    ...
                    type="file"
                    accept="image/*"
                    {...register(`photo${i}` as "photo1")}
                    className={
                      (errors as { [key: string]: unknown })[`photo${i}`]
                        ? "border-destructive"
                        : ""
                    }
                    disabled={isSubmitting}
                    ref={i === 1 ? photo1Ref : i === 2 ? photo2Ref : photo3Ref}
                  />
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className={
                    errors.ownershipDocument ? "border-destructive" : ""
                  }
                  disabled={isSubmitting}
                  ref={ownershipDocumentRef}
                />
            ...
            >
              ...
              {customerNotFound
                ? "Registrar Bici y Crear Cliente"
                : "Registrar Bici para Cliente"}
            </Button>
            <p className="text-xs text-muted-foreground">
              {!enteredCustomerEmail
                ? "Por favor, ingresa y verifica el correo del cliente primero."
                : isVerifyingCustomer
                  ? "Verificando cliente..."
                  : !verifiedCustomer && !customerNotFound
                    ? "Verifica el correo del cliente o ingresa sus datos manualmente si no se encuentra."
                    : ""}
            </p>
              ...
              <Card className="mt-4 p-4 bg-green-50 border-green-200">
                ...
                <CardContent className="p-0">
                  <p className="text-sm text-green-600">
                    Se han enviado instrucciones a{" "}
                  ...
                  <ul className="list-disc list-inside text-xs text-green-600 mt-1 space-y-0.5">
                    ...
                    <li>
                      Establecer su contraseña para acceder a BiciRegistro.
                    </li>
                  </ul>
                  <p className="text-xs text-green-500 mt-2">
                    Por favor, informa al cliente que revise su bandeja de
                    entrada (y spam).
                  </p>
                </CardContent>
              </Card>
