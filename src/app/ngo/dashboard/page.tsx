...
import {
  ...
  Download,
  BarChart,
  CalendarIcon as CalendarIconLucid,
  Share2,
  ClipboardCopy,
  UserCircle,
  PlusCircle,
...
export default function NgoDashboardPage() {
  ...
  const [analyticsData, setAnalyticsData] = useState<NgoAnalyticsData | null>(
    null,
  );
    ...
    try {
      ...
      setInviteTemplate(
        content?.ngoReferralMessageTemplate ||
          `¡Hola! En {{ngoName}} te invitamos a BiciRegistro. Regístrate aquí: {{ngoLink}}`,
      );
    } catch (error) {
  ...
  const handleInviteCommunity = () => {
    ...
    toast({
      ...
      description: "Se está abriendo WhatsApp para que envíes tu invitación.",
    });
  };
  ...
  const handleShareRideToWhatsApp = (ride: BikeRide) => {
    const formattedDate = format(new Date(ride.rideDate), "PPPP 'a las' p", {
      locale: es,
    });
    ...
    if (user?.whatsappGroupLink) {
      messageParts.push("");
      messageParts.push(
        `Para más detalles y unirte a nuestro grupo de WhatsApp: ${user.whatsappGroupLink}`,
      );
    }
    ...
    toast({
      title: "Listo para compartir",
      description:
        "Se está abriendo WhatsApp para que envíes el mensaje a tu grupo.",
    });
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm sm:text-base">
            Mide el impacto y gestiona los eventos de tu comunidad en
            BiciRegistro.
          </p>
        </div>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                {!user?.whatsappGroupLink && (
                  <TooltipContent>
                    <p>
                      Añade el enlace de tu grupo en tu perfil para activarlo.
                    </p>
                  </TooltipContent>
                )}
              </Tooltip>
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  ...
                  <CardDescription>
                    Crea, edita o elimina los eventos de tu comunidad.
                  </CardDescription>
                </div>
                    ...
                    >
                      <div className="flex-grow">
                        ...
                        <p className="text-sm text-muted-foreground flex items-center gap-2">
                          ...
                          {format(new Date(ride.rideDate), "PPPP 'a las' p", {
                            locale: es,
                          })}
                          {isPast(new Date(ride.rideDate)) && (
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="w-full sm:w-auto">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleShareRideToWhatsApp(ride)
                                  }
                                  disabled={!user?.whatsappGroupLink}
                                  className="w-full"
                                >
                            ...
                            {!user?.whatsappGroupLink && (
                              <TooltipContent>
                                <p>
                                  Añade el enlace de tu grupo en tu perfil para
                                  activarlo.
                                </p>
                              </TooltipContent>
                            )}
          ...
          <Card className="max-w-5xl mx-auto shadow-lg">
            <CardHeader>
              <CardTitle className="flex items-center text-2xl">
                <BarChart className="mr-3 h-7 w-7 text-primary" /> Estadísticas
                de tu Comunidad
              </CardTitle>
              <CardDescription>
                Visualiza métricas clave de los ciclistas registrados con tu
                enlace.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                <StatCard
                  ...
                  value={analyticsData?.totalUsersReferred ?? 0}
                  icon={<Users className="h-7 w-7 text-primary" />}
                  description="Ciclistas registrados con tu enlace."
                  isLoading={isLoading}
                />
                <StatCard
                  title="Bicis de tus Referidos"
                  value={analyticsData?.totalBikesFromReferrals ?? 0}
                  icon={<Bike className="h-7 w-7 text-primary" />}
                  description="Total de bicis de tu comunidad."
                  isLoading={isLoading}
                />
                <StatCard
                  title="Reportes de Robo"
                  value={analyticsData?.stolenBikesFromReferrals ?? 0}
                  icon={<ShieldAlert className="h-7 w-7 text-destructive" />}
                  description="Bicis de tu comunidad reportadas."
                  isLoading={isLoading}
                />
                <StatCard
                  title="Bicis Recuperadas"
                  value={analyticsData?.recoveredBikesFromReferrals ?? 0}
                  icon={<CheckCircle className="h-7 w-7 text-green-600" />}
                  description="Reportes de robo resueltos."
                  isLoading={isLoading}
                />
              </div>
      ...
      <AlertDialog
        open={!!rideToDelete}
        onOpenChange={(open) => !open && setRideToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
            ...
            <AlertDialogDescription>
              ...
              evento:{" "}
              <strong className="break-all">{rideToDelete?.title}</strong>.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
