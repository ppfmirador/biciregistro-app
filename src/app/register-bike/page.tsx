  ...
  useEffect(() => {
    if (!authLoading && (!user || user.isAnonymous)) {
      toast({
        ...
        description: "Por favor, inicia sesión para registrar una bicicleta.",
        variant: "destructive",
      });
      router.push("/auth");
    }
  ...
  const handleSubmit = async (data: BikeFormValues) => {
    if (!user || user.isAnonymous) {
      toast({
        ...
        description: "Debes iniciar sesión para registrar una bicicleta.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const photoUrls = uploadResults
        .filter((r): r is { type: "photo"; url: string } => r?.type === "photo")
        .map((r) => r.url);
      const docResult = uploadResults.find(
        (r): r is { type: "document"; url: string; name: string } =>
          r?.type === "document" && !!r.name,
      );
      ...
      Object.keys(newBikeData).forEach((key) => {
        if (
          newBikeData[key] === undefined ||
          newBikeData[key] === null ||
          newBikeData[key] === ""
        ) {
      ...
      const createBikeCallable = httpsCallable<
        Record<string, unknown>,
        { bikeId: string }
      >(functions, "createBike");
      await createBikeCallable({ bikeData: newBikeData });

      toast({
        title: "¡Bicicleta Registrada!",
        description: `${finalBrand} ${data.model} ha sido registrada exitosamente.`,
      });
      ...
      <Card className="max-w-2xl mx-auto shadow-xl">
        <CardHeader>
          ...
          <CardDescription className="text-sm sm:text-base">
            Completa los detalles a continuación para agregar tu bicicleta al
            registro. Puedes subir hasta 3 fotos y un documento de propiedad
        ...
        <CardContent>
          <BikeForm
            ...
            isLoading={isLoading}
            submitButtonText="Registrar Bici"
          />
        </CardContent>
      </Card>
    </div>
