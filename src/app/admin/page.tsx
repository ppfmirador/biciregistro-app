...
const homepageTextContentSchema = z.object({
  ...
  welcomeDescription: z
    .string()
    .min(1, "La descripción de bienvenida es obligatoria."),
  whyAppNameTitle: z
    .string()
    .min(1, 'El título de la sección "¿Por qué?" es obligatorio.'),
  feature1Title: z
    .string()
    .min(1, "El título de la Característica 1 es obligatorio."),
  feature1Description: z
    .string()
    .min(1, "La descripción de la Característica 1 es obligatoria."),
  feature2Title: z
    .string()
    .min(1, "El título de la Característica 2 es obligatorio."),
  feature2Description: z
    .string()
    .min(1, "La descripción de la Característica 2 es obligatoria."),
  feature3Title: z
    .string()
    .min(1, "El título de la Característica 3 es obligatorio."),
  feature3Description: z
    .string()
    .min(1, "La descripción de la Característica 3 es obligatoria."),
  communityTitle: z
    .string()
    .min(1, "El título de la sección de comunidad es obligatorio."),
  communityDescription: z
    .string()
    .min(1, "La descripción de la sección de comunidad es obligatoria."),
  referralMessage: z.string().optional(),
  ngoReferralMessageTemplate: z
    .string()
    .max(500, "La plantilla no debe exceder los 500 caracteres.")
    .optional(),
});
...
export default function AdminPage() {
  ...
  const [communityImagePreview, setCommunityImagePreview] = useState<
    string | undefined
  >(defaultContentValues.communityImageUrl);
  const [sponsors, setSponsors] = useState<SponsorAdminItem[]>([]);
  const [existingCommunityImageUrl, setExistingCommunityImageUrl] = useState<
    string | undefined
  >();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<HomepageTextFormValues>({
    ...
    try {
      ...
      if (content) {
        reset({
          ...
          referralMessage:
            content.referralMessage || defaultContentValues.referralMessage,
          ngoReferralMessageTemplate:
            content.ngoReferralMessageTemplate ||
            defaultContentValues.ngoReferralMessageTemplate,
        });
        ...
        setSponsors(
          content.sponsors.map((s) => ({
            ...s,
            existingLogoPath: getPathFromStorageUrl(s.logoUrl),
          })),
        );
      } else {
        ...
        setSponsors(
          defaultContentValues.sponsors.map((s) => ({
            ...s,
            existingLogoPath: getPathFromStorageUrl(s.logoUrl),
          })),
        );
      ...
      toast({
        title: "Error",
        description:
          "No se pudo cargar el contenido/usuarios del administrador.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSponsorChange = (
    ...
    field: keyof Omit<
      SponsorAdminItem,
      "id" | "existingLogoPath" | "dataAiHint"
    >,
    value: string | File,
  ) => {
    ...
    if (field === "logoFile" && value instanceof File) {
      ...
      typeof value === "string" &&
      (field === "name" || field === "logoUrl" || field === "link")
    ) {
  ...
  const onSubmitHomepageContent: SubmitHandler<HomepageTextFormValues> = async (
    data,
  ) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (communityImageFile) {
        if (existingCommunityImageUrl) {
          ...
          if (oldPath)
            await deleteFileFromStorage(oldPath).catch((err) =>
              console.warn("Old community image deletion failed:", err),
            );
        }
        const timestamp = Date.now();
        const extension = communityImageFile.name.split(".").pop();
        finalCommunityImageUrl = await uploadFileToStorage(
          communityImageFile,
          `homepage_assets/community_image_${timestamp}.${extension}`,
        );
      }
      ...
      for (const sponsor of sponsors) {
        ...
        if (sponsor.logoFile) {
          if (sponsor.existingLogoPath) {
            await deleteFileFromStorage(sponsor.existingLogoPath).catch((err) =>
              console.warn(
                `Sponsor logo deletion failed for ${sponsor.name}:`,
                err,
              ),
            );
          }
          ...
          currentLogoUrl = await uploadFileToStorage(
            sponsor.logoFile,
            logoPath,
          );
        }
      ...
      for (const initialSponsor of initialSponsors) {
        if (!finalSponsors.find((fs) => fs.id === initialSponsor.id)) {
          ...
          if (oldLogoPath) {
            await deleteFileFromStorage(oldLogoPath).catch((err) =>
              console.warn(
                `Orphaned sponsor logo deletion failed for ${initialSponsor.name}:`,
                err,
              ),
            );
          }
      ...
      const homepageDataToSave = {
        ...
        referralMessage:
          data.referralMessage || defaultContentValues.referralMessage,
        ngoReferralMessageTemplate:
          data.ngoReferralMessageTemplate ||
          defaultContentValues.ngoReferralMessageTemplate,
        communityImageUrl:
          finalCommunityImageUrl || defaultContentValues.communityImageUrl,
      ...
      toast({
        ...
        description: "El contenido de la página principal ha sido guardado.",
      });
  ...
  const handleApplyFilters = () => {
    ...
    toast({
      ...
      description:
        "La carga de estadísticas con filtros aún no está implementada.",
    });
  ...
  const handleUpdateRole = async (uid: string, role: UserRole) => {
    ...
    try {
      ...
      const errorMessage =
        err.message || "Error desconocido al actualizar rol.";
  ...
  const handleShopFormSubmit = async (data: BikeShopAdminFormValues) => {
    ...
    try {
      ...
      const errorMessage =
        err.message || "Error desconocido al guardar tienda.";
      toast({
          ...
          <CardContent>
            {authLoading ? (
              ...
              <p className="text-center text-destructive">
                No tienes permisos para ver esta página o no has iniciado sesión
                como administrador.
              </p>
            )}
  ...
  return (
    <div className="space-y-8">
      ...
      <p className="text-center text-muted-foreground text-sm">
        Bienvenido, {user?.email}. Gestiona el contenido y la configuración de
        la aplicación.
      </p>
        ...
        <TabsContent value="contentManagement">
          <Card className="max-w-3xl mx-auto shadow-lg mt-6">
            <CardHeader>
              ...
              <CardDescription>
                Modifica los textos y las imágenes de la página principal.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form
                onSubmit={handleSubmit(onSubmitHomepageContent)}
                className="space-y-6"
              >
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={errors.welcomeTitle ? "border-destructive" : ""}
                  />
                  {errors.welcomeTitle && (
                    <p className="text-sm text-destructive">
                      {errors.welcomeTitle.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className={
                      errors.welcomeDescription ? "border-destructive" : ""
                    }
                    rows={3}
                  />
                ...
                <h2 className="text-xl font-semibold pt-4 border-t">
                  Sección &quot;¿Por qué BiciRegistro?&quot;
                </h2>
                <div className="space-y-2">
                  <Label htmlFor="whyAppNameTitle">Título de la Sección</Label>
                  <Input
                    id="whyAppNameTitle"
                    {...register("whyAppNameTitle")}
                    className={
                      errors.whyAppNameTitle ? "border-destructive" : ""
                    }
                  />
                  {errors.whyAppNameTitle && (
                    <p className="text-sm text-destructive">
                      {errors.whyAppNameTitle.message}
                    </p>
                  )}
                </div>
                <h3 className="text-lg font-medium">
                  Característica 1 (Registro Seguro)
                </h3>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={errors.feature1Title ? "border-destructive" : ""}
                  />
                  {errors.feature1Title && (
                    <p className="text-sm text-destructive">
                      {errors.feature1Title.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  <Label htmlFor="feature1Description">
                  ...
                  <Textarea
                    ...
                    className={
                      errors.feature1Description ? "border-destructive" : ""
                    }
                    rows={3}
                  />
                  {errors.feature1Description && (
                    <p className="text-sm text-destructive">
                      {errors.feature1Description.message}
                    </p>
                  )}
                </div>

                <h3 className="text-lg font-medium pt-2">
                  Característica 2 (Reporte de Robo)
                </h3>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={errors.feature2Title ? "border-destructive" : ""}
                  />
                  {errors.feature2Title && (
                    <p className="text-sm text-destructive">
                      {errors.feature2Title.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  <Label htmlFor="feature2Description">
                  ...
                  <Textarea
                    ...
                    className={
                      errors.feature2Description ? "border-destructive" : ""
                    }
                    rows={3}
                  />
                  {errors.feature2Description && (
                    <p className="text-sm text-destructive">
                      {errors.feature2Description.message}
                    </p>
                  )}
                </div>

                <h3 className="text-lg font-medium pt-2">
                  Característica 3 (Vigilancia Comunitaria)
                </h3>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={errors.feature3Title ? "border-destructive" : ""}
                  />
                  {errors.feature3Title && (
                    <p className="text-sm text-destructive">
                      {errors.feature3Title.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2">
                  <Label htmlFor="feature3Description">
                    Descripción Característica 3
                  </Label>
                  <Textarea
                    id="feature3Description"
                    {...register("feature3Description")}
                    className={
                      errors.feature3Description ? "border-destructive" : ""
                    }
                    rows={3}
                  />
                  {errors.feature3Description && (
                    <p className="text-sm text-destructive">
                      {errors.feature3Description.message}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={
                      errors.communityTitle ? "border-destructive" : ""
                    }
                  />
                  {errors.communityTitle && (
                    <p className="text-sm text-destructive">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className={
                      errors.communityDescription ? "border-destructive" : ""
                    }
                    rows={3}
                  />
                ...
                <div className="space-y-2">
                  <Label htmlFor="communityImageUpload">
                    Imagen de Comunidad
                  </Label>
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    JPG/PNG/WEBP, máx 1MB recomendado.
                  </p>
                </div>

                <h2 className="text-xl font-semibold pt-4 border-t">
                  Mensajes de Invitación por WhatsApp
                </h2>
                <div className="space-y-2">
                  <Label htmlFor="referralMessage">
                    Mensaje para Ciclistas
                  </Label>
                  <Textarea
                    id="referralMessage"
                    ...
                    className={
                      errors.referralMessage ? "border-destructive" : ""
                    }
                  />
                  {errors.referralMessage && (
                    <p className="text-sm text-destructive">
                      {errors.referralMessage.message}
                    </p>
                  )}
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-2">
                  <Label htmlFor="ngoReferralMessageTemplate">
                    Plantilla de Invitación para ONGs
                  </Label>
                  <Textarea
                    ...
                    className={
                      errors.ngoReferralMessageTemplate
                        ? "border-destructive"
                        : ""
                    }
                  />
                  {errors.ngoReferralMessageTemplate && (
                    <p className="text-sm text-destructive">
                      {errors.ngoReferralMessageTemplate.message}
                    </p>
                  )}
                  <p className="text-xs text-muted-foreground">
                    Placeholders permitidos: <strong>{"{{ngoName}}"}</strong>,{" "}
                    <strong>{"{{ngoLink}}"}</strong>.
                  </p>
                </div>
                ...
                {sponsors.map((sponsor, index) => (
                  <Card key={sponsor.id} className="p-4 space-y-3 my-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`sponsorName-${index}`}>
                        Nombre del Aliado
                      </Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`sponsorLink-${index}`}>
                        Enlace (URL) del Aliado (Opcional)
                      </Label>
                      <Input
                        ...
                        value={sponsor.link || ""}
                        onChange={(e) =>
                          handleSponsorChange(index, "link", e.target.value)
                        }
                        placeholder="https://ejemplo.com"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor={`sponsorDataAiHint-${index}`}>
                        Palabras clave para IA (logo)
                      </Label>
                      <Input
                        id={`sponsorDataAiHint-${index}`}
                        value={sponsor.dataAiHint || ""}
                        onChange={(e) =>
                          handleSponsorDataAiHintChange(index, e.target.value)
                        }
                        placeholder="ej. company logo"
                      />
                      <p className="text-xs text-muted-foreground">
                        Max 2 palabras, para búsqueda de imágenes.
                      </p>
                    </div>
                    <div className="space-y-2">
                      ...
                      {(sponsor.newLogoPreview || sponsor.logoUrl) && (
                        <div className="my-2">
                          <NextImage
                            src={sponsor.newLogoPreview || sponsor.logoUrl}
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        PNG/JPG/WEBP, máx 200KB recomendado.
                      </p>
                    </div>
                  </Card>
                ))}
                ...
                <Button
                  ...
                  disabled={isLoading || isFetchingAdminContent}
                >
                  {(isLoading || isFetchingAdminContent) && (
          ...
          <Card className="max-w-5xl mx-auto shadow-lg mt-6">
            <CardHeader>
              <CardTitle className="flex items-center text-2xl">
                <LayoutGrid className="mr-3 h-7 w-7 text-primary" /> Panel de
                Estadísticas
              </CardTitle>
              <CardDescription>
                Visualiza métricas clave y filtra datos del uso de la
                aplicación.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-8">
              <Card className="p-4 sm:p-6">
                ...
                <CardContent className="p-0 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      <Label htmlFor="dateRangeStart" className="text-sm">
                        Rango de Fechas
                      </Label>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Funcionalidad de filtrado por fecha próximamente.
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      <Label htmlFor="locationFilter" className="text-sm">
                        Por Ubicación (Estado)
                      </Label>
                      <Select
                        value={selectedLocation}
                        onValueChange={(value) =>
                          setSelectedLocation(
                            value === ALL_LOCATIONS_VALUE ? "" : value,
                          )
                        }
                        disabled={isDashboardLoading}
                      >
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Funcionalidad de filtrado por ubicación próximamente.
                      </p>
                    </div>
                  </div>
                  <Button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                <StatCard
                  ...
                  value="N/A"
                  icon={<BikeIcon className="h-7 w-7 text-primary" />}
                  description="Total de bicis en el sistema."
                ...
                />
                <StatCard
                  title="Reportes de Robo Activos"
                  value="N/A"
                  icon={<AlertTriangle className="h-7 w-7 text-destructive" />}
                  description="Bicis actualmente marcadas como robadas."
                  isLoading={isDashboardLoading}
                />
                <StatCard
                  title="Transferencias Completadas"
                  value="N/A"
                  icon={<ArrowRightLeft className="h-7 w-7 text-green-600" />}
                  description="Total de transferencias de propiedad exitosas."
                  isLoading={isDashboardLoading}
                />
              </div>
              ...
              <Card className="p-4 sm:p-6 text-center">
                <CardHeader className="p-0 pb-4">
                  <CardTitle className="text-xl flex items-center justify-center">
                    <BarChart className="mr-2 h-6 w-6 text-primary" /> Gráficas
                    de Uso
                  </CardTitle>
                </CardHeader>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <CardTitle className="flex items-center">
                    <Store className="mr-3 h-6 w-6 text-primary" /> Gestionar
                    Tiendas de Bicicletas
                  </CardTitle>
                  <CardDescription>
                    Crea, edita y gestiona las cuentas de tiendas de bicicletas.
                  </CardDescription>
                </div>
                    ...
                    <Card key={shop.id} className="p-3">
                      <div className="flex justify-between items-center">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleOpenShopForm(shop)}
                        >
              ...
              <div className="flex justify-between items-center">
                <div>
                  <CardTitle className="flex items-center">
                    ...
                    Gestionar ONGs y Colectivos
                  </CardTitle>
                  <CardDescription>
                    Crea, edita y gestiona las cuentas de ONGs.
                  </CardDescription>
                </div>
        ...
        <TabsContent value="userManagement">
          <Card className="max-w-4xl mx-auto shadow-lg mt-6">
            <CardHeader>
              ...
              <CardDescription>
                Administra los roles y el estado de todos los usuarios de la
                plataforma.
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <Card key={userItem.id} className="p-3">
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          <Select
                            value={userItem.role || "cyclist"}
                            onValueChange={(role) =>
                              handleUpdateRole(userItem.id, role as UserRole)
                            }
                            disabled={isLoading}
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="bikeshop">
                                <Store className="inline-block mr-2 h-4 w-4" />
                                Tienda
                              </SelectItem>
                              <SelectItem value="ngo">
                                <HeartHandshake className="inline-block mr-2 h-4 w-4" />
                                ONG
                              </SelectItem>
                          ...
                          <Button
                            ...
                            disabled={isLoading || userItem.id === user?.uid}
                          >
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
      ...
      <Dialog open={isShopFormOpen} onOpenChange={setIsShopFormOpen}>
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {editingShop
                ? "Editar Tienda de Bicicletas"
                : "Añadir Nueva Tienda de Bicicletas"}
            </DialogTitle>
            <DialogDescription>
              {editingShop
                ? `Modifica los detalles de ${editingShop.shopName}.`
                : "Completa el formulario para registrar una nueva tienda. Se creará una cuenta y se enviará un correo para establecer la contraseña."}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isNgoFormOpen} onOpenChange={setIsNgoFormOpen}>
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {editingNgo ? "Editar ONG" : "Añadir Nueva ONG"}
            </DialogTitle>
            <DialogDescription>
              {editingNgo
                ? `Modifica los detalles de ${editingNgo.ngoName}.`
                : "Completa el formulario para registrar una nueva ONG. Se creará una cuenta y se enviará un correo para establecer la contraseña."}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 max-h-[70vh] overflow-y-auto pr-4">
            <NgoForm
      ...
      >
        <AlertDialogContentCustom>
          <AlertDialogHeaderCustom>
            <AlertDialogTitleCustom>
              ¿Estás absolutamente seguro?
            </AlertDialogTitleCustom>
            <AlertDialogDescriptionCustom>
              Esta acción es irreversible. Se eliminará permanentemente la
              cuenta de{" "}
              <strong className="break-all">{userToDelete?.email}</strong> y
              todos sus datos asociados, incluidas las bicicletas que haya
              registrado.
              {userToDelete &&
                typeof userToDelete.bikeCount === "number" &&
                userToDelete.bikeCount > 0 && (
                  <span className="font-bold text-destructive block mt-2">
                    ¡Advertencia! Este usuario tiene {userToDelete.bikeCount}{" "}
                    bicicleta(s) registrada(s) que también serán eliminadas.
                  </span>
                )}
            </AlertDialogDescriptionCustom>
          </AlertDialogHeaderCustom>
