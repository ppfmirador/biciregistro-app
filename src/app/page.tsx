...
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    fetchedContent = await getHomepageContent();
  } catch (error) {
    console.error("Error fetching homepage content for metadata:", error);
  }
  const title = fetchedContent?.welcomeTitle || defaultContent.welcomeTitle.replace('{APP_NAME}', APP_NAME);
  const description = fetchedContent?.welcomeDescription || defaultContent.welcomeDescription;
...
export default async function Page() {
  ...
  try {
    ...
    console.error("Error fetching homepage content for server component:", error);
