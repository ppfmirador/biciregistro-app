...
function QrCodePageContent() {
  ...
  const fetchBikeData = useCallback(async () => {
    setIsLoading(true);
    try {
      const bikeData = await getBikeBySerialNumber(
      ...
      if (bikeData.ownerId !== user?.uid) {
        ...
        toast({
          ...
          description:
            "No tienes permiso para gestionar el QR de esta bicicleta.",
          variant: "destructive",
        });
        router.push("/dashboard");
        return;
    ...
    return (
      <div className="text-center py-10">
        <Alert variant="destructive" className="max-w-md mx-auto">
          ...
          <AlertDescription>
            Bicicleta no encontrada o no tienes permiso para acceder a esta
            página.
          </AlertDescription>
        </Alert>
  ...
  const displayPhotoUrl =
    bike.photoUrls && bike.photoUrls.filter((url) => url).length > 0
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <Button
        variant="outline"
        onClick={() => router.push(`/bike/${serialNumber}`)}
        ...
        <CardHeader>
          <div className="flex items-center gap-2 mb-2">
            ...
            <CardTitle className="text-2xl sm:text-3xl font-headline">
              Código QR de tu Bicicleta
            </CardTitle>
          </div>
          <CardDescription className="text-sm sm:text-base">
            Este código QR enlaza al perfil público de tu {bike.brand}{" "}
            {bike.model}. Descárgalo y pégalo en tu bicicleta para una fácil
        ...
        <CardContent className="grid items-start md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <Card className="overflow-hidden">
              ...
              <div className="p-4 space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  <span className="font-semibold">N/S:</span>{" "}
                  {bike.serialNumber}
                </p>
            ...
            <Alert>
              ...
              <AlertDescription>
                La información del propietario (tu nombre, contacto, etc.) NUNCA
                se muestra en el perfil público. Solo se muestra el estado
                actual de la bicicleta.
              </AlertDescription>
            </Alert>
          </div>
          <div className="space-y-4">
