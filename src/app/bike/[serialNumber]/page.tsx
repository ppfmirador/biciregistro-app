...
function BikeDetailsPageContent() {
  ...
  const fetchBike = React.useCallback(async () => {
    if (serialNumberParam) {
      ...
      try {
        ...
        const checkIsOwner = !!(bikeData && user && bikeData.ownerId === user.uid);
  ...
  const handlePhotoFileChange = async (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
    if (!event.target.files || event.target.files.length === 0 || !bike || !user || bike.ownerId !== user.uid) {
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Error al Subir Foto', description: `No se pudo subir la foto ${index + 1}.`, variant: 'destructive' });
    } finally {
      setUploadingPhoto(prev => {
        const newUploading = [...prev];
  ...
  const handleRemovePhoto = async (index: number) => {
    if (!bike || !user || bike.ownerId !== user.uid || !bike.photoUrls || !bike.photoUrls[index]) {
      return;
    }
     setUploadingPhoto(prev => {
      const newUploading = [...prev];
    ...
    try {
      ...
      toast({ title: 'Error al Eliminar Foto', description: `No se pudo eliminar la foto ${index + 1}.`, variant: 'destructive' });
    } finally {
       setUploadingPhoto(prev => {
        const newUploading = [...prev];
        newUploading[index] = false;
  ...
  const handleDocumentFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: 'Error al Subir Documento', description: 'No se pudo subir el documento.', variant: 'destructive' });
    } finally {
  ...
  const handleRemoveDocument = async () => {
    if (!bike || !user || bike.ownerId !== user.uid || !bike.ownershipDocumentUrl) {
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Error al Eliminar Documento', description: 'No se pudo eliminar el documento.', variant: 'destructive' });
    } finally {
