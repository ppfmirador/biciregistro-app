    ...
    try {
      ...
      if (bikeData.ownerId !== user?.uid) {
        toast({
          ...
          description: "No tienes permiso para editar esta bicicleta.",
          variant: "destructive",
        });
  ...
  const handleSubmit = async (data: BikeFormValues) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Los detalles de tu bicicleta han sido actualizados.",
      });
      router.push(
        `/bike/${encodeURIComponent(data.serialNumber || bike.serialNumber)}`,
      );
    } catch (error: unknown) {
      const errorMessage =
        error instanceof Error
      ...
      <Card className="max-w-2xl mx-auto shadow-xl">
        <CardHeader>
          ...
          <CardDescription className="text-sm sm:text-base">
            Modifica la información de tu {bike.brand} {bike.model}. Para
            cambiar fotos o el documento de propiedad, usa la página de
            detalles.
          </CardDescription>
        </CardHeader>
        <CardContent>
