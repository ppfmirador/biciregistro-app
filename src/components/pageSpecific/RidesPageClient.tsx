        ...
        <div className="space-y-2 text-sm">
          ...
          {ride.state && ride.country && (
            <div className="flex items-center gap-2">
              ...
              <span>
                <span className="font-medium">Ubicación:</span> {ride.state},{" "}
                {ride.country}
              </span>
            </div>
          )}
          ...
          {ride.level && (
            <div className="flex items-center gap-2">
              ...
              <span>
                <span className="font-medium">Nivel:</span> {ride.level}
              </span>
            </div>
          )}
          {ride.modality && (
            <div className="flex items-center gap-2">
              ...
              <span>
                <span className="font-medium">Modalidad:</span> {ride.modality}
              </span>
            </div>
          )}
          <div className="flex items-center gap-2">
            ...
            <span>
              ...
              {ride.cost && ride.cost > 0
                ? `$${ride.cost.toFixed(2)} MXN`
                : "Gratuito"}
            </span>
          </div>
        </div>
      ...
      <div className="p-6 pt-0">
        {user ? (
          ride.meetingPointMapsLink && (
            <a
              href={ride.meetingPointMapsLink}
              target="_blank"
...
export default function RidesPageClient({
  ...
  const availableStates = useMemo(() => {
    ...
    return (
      LAT_AM_LOCATIONS.find((loc) => loc.country === selectedCountry)?.states ||
      []
    );
  }, [selectedCountry]);
  ...
  const filteredRides = useMemo(() => {
    return initialRides.filter((ride) => {
      const countryMatch =
        selectedCountry === ALL_VALUE || ride.country === selectedCountry;
      const stateMatch =
        selectedState === ALL_VALUE || ride.state === selectedState;
      const modalityMatch =
        selectedModality === ALL_VALUE || ride.modality === selectedModality;
      const levelMatch =
        selectedLevel === ALL_VALUE || ride.level === selectedLevel;
    ...
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-3xl sm:text-4xl font-headline font-bold text-primary">
          Próximos Eventos Seguros
        </h1>
        <p className="mt-2 text-lg text-muted-foreground">
          Encuentra y únete a los eventos organizados por ONGs y colectivos
          ciclistas.
        </p>
      </div>
      ...
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>
            Usa los filtros para encontrar el evento perfecto para ti.
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                disabled={selectedCountry === ALL_VALUE}
              >
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {BIKE_TYPES.filter((bt) => bt.value).map((bt) => (
            ...
            <div className="space-y-1">
              ...
              <Select value={selectedLevel} onValueChange={setSelectedLevel}>
                ...
                <SelectContent>
                  ...
                  {RIDE_LEVELS.filter((level) => level.value).map((level) => (
      ...
      {filteredRides.length > 0 ? (
        ...
        <div className="text-center py-16 border-2 border-dashed rounded-lg">
          ...
          <p className="text-muted-foreground mt-2">
            Intenta ajustar o limpiar los filtros para ver más eventos.
          </p>
        </div>
      )}
    </div>
