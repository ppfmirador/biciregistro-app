...
const BikeCompletenessIndicator: React.FC<BikeCompletenessIndicatorProps> = ({
  bike,
  className,
}) => {
  if (!bike) return null;
    ...
    <div className={className}>
      ...
      {completeness < 100 && (
        <ul className="mt-2 text-xs text-muted-foreground list-disc list-inside space-y-0.5">
          {!(bike.serialNumber && bike.brand && bike.model) && (
            <li>+30% por detalles básicos (N/S, Marca, Modelo).</li>
          )}
          {!(
            bike.photoUrls &&
            bike.photoUrls.filter((url) => url && url.trim() !== "").length > 0
          ) && <li>+30% por subir al menos una foto.</li>}
          {!(
            bike.ownershipDocumentUrl && bike.ownershipDocumentUrl.trim() !== ""
          ) && <li>+40% por subir documento de propiedad.</li>}
        </ul>
      )}
    </div>
  );
