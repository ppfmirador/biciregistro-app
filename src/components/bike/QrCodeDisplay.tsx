...
const QrCodeDisplay: React.FC<QrCodeDisplayProps> = ({ bike }) => {
  ...
  const handleDownloadPng = () => {
    ...
    if (!canvas) {
      toast({
        title: "Error",
        description: "No se pudo generar el código QR para PNG.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDownloadPdf = () => {
    ...
    if (!canvas) {
      toast({
        title: "Error",
        description: "No se pudo generar el código QR para el PDF.",
        variant: "destructive",
      });
      return;
    }
    ...
    doc.setFontSize(6);
    doc.setFont("helvetica", "normal");
    doc.text(`Verifica en: ${SITE_URL.replace("https://", "")}`, 25, 96, {
      align: "center",
    });
