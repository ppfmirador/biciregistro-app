    ...
    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col overflow-hidden">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-6 w-6 text-white hover:bg-white/20 p-0.5"
                    >
      ...
      <CardContent className="p-3 sm:p-4 flex-grow space-y-2">
        <div>
          ...
          <CardDescription className="text-xs sm:text-sm text-muted-foreground mb-1">
            N/S: {bike.serialNumber}
          </CardDescription>
          <p className="text-xs text-muted-foreground">
            Registrada:{" "}
            {format(new Date(bike.registrationDate), "d MMM, yy", {
              locale: es,
            })}
          </p>
          {bike.color && (
            <p className="text-xs text-muted-foreground">Color: {bike.color}</p>
          )}
          {bike.state && (
            <p className="text-xs text-muted-foreground">
              Estado: {bike.state}
