...
const SearchBikeForm: React.FC<SearchBikeFormProps> = ({
  initialSerialNumber,
}) => {
  ...
  const onSubmit: SubmitHandler<SearchFormValues> = async (data) => {
    ...
    try {
      ...
      router.push(
        `/bike/${encodeURIComponent(data.serialNumber.trim())}?from=home`,
      );
    } catch (_error: unknown) {
      toast({
        title: "Error en la Búsqueda",
        description:
          "No se pudo procesar la búsqueda. Por favor, inténtalo de nuevo.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
      <div>
        ...
        {errors.serialNumber && (
          <p id="serialNumberError" className="mt-2 text-sm text-destructive">
