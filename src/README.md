...
To manage the platform, you need an administrator account. This requires manually executing a Cloud Function.

1.  **Deploy the project:** Make sure all your functions are deployed to the target environment (e.g., staging).
    ```bash
    npm run deploy:staging
    ```
2.  **Create a standard user account:** Go to your deployed app's URL, navigate to "Crear Cuenta", and register the user you want to be the administrator.
3.  **Get the user's UID:**
    *   Open the [Firebase Console](https://console.firebase.google.com/) and select your project.
    *   Go to **Authentication > Users**.
    *   Find the user you just created and copy their **UID**.
4.  **Execute the `updateUserRole` function:**
    *   In the Firebase Console, go to **Functions**.
    *   Find `updateUserRole` in the list and click on it.
    *   Navigate to the **TESTING** tab.
    *   In the "Test data (JSON)" field, enter the following, replacing `<UID_DEL_USUARIO>` with the UID you copied:
      ```json
      {
        "data": {
          "uid": "<UID_DEL_USUARIO>",
      ...
      ```
    *   **Crucially**, to bypass the admin-only security rule for this one time, you must simulate an admin call. In the "Authentication configuration" section below the test data, check the **"Authenticated request"** box. In the JSON object that appears, add the admin claim:
      ```json
      {
        "auth": {
          "token": {
      ...
      ```
    *   Click **"Test function"**. You should see a success message.
5.  **Verify Access:** Log out and log back into the application with the user you just promoted. You should now have access to the `/admin` panel.
...
- **Firebase Auth Redirects:** For authentication to work correctly on a custom domain, the `next.config.mjs` file contains a conditional rewrite rule. Ensure the `FIREBASE_AUTH_HOSTING_URL` environment variable is set correctly in your production hosting environment.
