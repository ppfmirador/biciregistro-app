...
const handleCreateBike = async (data: { bikeData: Partial<Bike> & { registeredByShopId?: string | null } }, context: AuthContext) => {
  ...
  const dataToSave: Omit<Bike, 'id' | 'registrationDate'> & { registrationDate: admin.firestore.Timestamp, statusHistory: admin.firestore.FieldValue } = {
    ...
    statusHistory: admin.firestore.FieldValue.arrayUnion({ status: "En Regla", timestamp: admin.firestore.Timestamp.now(), notes: bikeData.registeredByShopId ? `Registrada por tienda: ${ownerData?.shopName || 'Tienda'}` : "Registro inicial por ciclista" }),
    theftDetails: null,
...
const handleReportBikeStolen = async (data: { bikeId: string; theftData: { theftLocationState: string; theftLocationCountry?: string; theftPerpetratorDetails?: string; theftIncidentDetails: string; generalNotes?: string; }; }, context: AuthContext) => {
